...
describe('unit:utils/resolveAlias', () => {
  ...
  it('should convert alias from unknown parent to absolute specifier', () => {
    // Act
    const result = testSubject('#src/utils/detect-syntax', { paths })

    // Expect
    expect(result).to.equal(path.join(process.cwd(), 'src/utils/detect-syntax'))
  })
  ...
  it('should convert module alias to relative specifier', () => {
    ...
    const parent = path.resolve('src/utils/__tests__/detect-syntax.spec.ts')
